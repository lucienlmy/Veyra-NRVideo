...
2026-09-22 用户确认本地测试包可用，要求"发布 1.4.4 到 GitHub、写好更新报告、README 只留最新版简易报告、
老版本部分删掉、细节看 Release"，并授权推送与发布。
...
- 本轮新增两个发布前提交：`d93864f`（严格补帧节奏行重排、滚动提交重绘 + 新回归测试）、
  `6851c27`（README 精简、1.4.4 更新报告、交流群二维码）。
...
- 发布后核验：`releases/latest` 返回 `v1.4.4`（draft=false、prerelease=false）；
  4 个下载地址与两个二维码图片地址全部 HTTP 200；远端 `README.md` / `README_EN.md` 只保留 1.4.4 更新，
