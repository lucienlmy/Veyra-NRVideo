...
std::wstring smokePacing;bool smokePacingVsync=false,smokePacingApplied=false;
...
if(!smokePacingApplied&&!smokePacing.empty()&&s.frames>0){smokePacingApplied=true;
    veyra::engine::PresentationSettings p{};
    p.enabled=smokePacing!=L"off";
    p.mode=smokePacing==L"even"?veyra::engine::PacingMode::Even:smokePacing==L"reflex"?veyra::engine::PacingMode::Reflex:veyra::engine::PacingMode::LowQueue;
...
// anywhere on the command line instead of only at the end.
{
   ...
   const wchar_t* valueFlags[]={L"--export-worker",L"--smoke-view",L"--smoke-pacing",L"--smoke-dual-export",L"--smoke-seconds",L"--export-out",L"--max-frames",L"--cancel-after-ms",L"--smoke-save",L"--fg-multiplier",L"--capture-audio",L"--video-sr",L"--bitrate-mbps",L"--subtitle-primary",L"--subtitle-secondary",L"--subtitle-offset-ms",L"--subtitle-font-size"};
